...
**最少输入**：链接（或 ASIN）+ **目标国家/站点**。有这两样，十个维度里的九个都能跑完。
**推荐补一张产品主图**（剪贴板直接粘贴）：有它版权维度才算数，没有就标「图源未确认」。
**以下情况必须给图**：① 你要查的是**自己的货**而不是这个 listing（页面图是竞品的，和你 1688 拿的未必同形态）；② 页面抓不到（反爬机器人页）；③ 要判「成品 vs 内胆」这类细节。

建议同时给出**已知的拿货来源**（1688 同款 / ODM 定制），便于判断"能做 ≠ 好做"。
